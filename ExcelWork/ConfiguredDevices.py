# -*- coding: utf-8 -*-
"""
Created on Wed Mar 29 14:24:18 2017
...
def printdevices(Zones, DeviceName):
    ...
    print("Printing for Device ->",DeviceName)
        ...
        if (not (row[0].value is None) and not (row[1].value is None)):
            if(CurrZone!=row[0].value and not (CurrZone is None)):
                ...
                def forallDevices(Device_Pars, Device_Zone, Device_Zones):
                    #print(Device_Pars, Device_Zones, Device_Zone)
                ...
                for Device in AllDevices:
                    ...
                    #exec("if len(%s_Pars) > 0:\n\t%s_Zone = Zone.copy()\n"% (Device,Device))
                    #exec("%s_Zone['Pars'] = %s_Pars.copy()"% (Device,Device))
                    #exec("%s_Pars.clear()"% Device)

                    #if len(exec("%s_Zone"% Device)) > 0:
                    #    exec("%s_Zones[CurrZone]=%s_Zone.copy()"%(Device,Device))
...
if (fileFormatValid == 1):    
    ...
    for Device in AllDevices:
        exec("printdevices(%s_Zones,'%s')" % (Device,Device))
    #print("###########################  AG_VP_MC_22_Zones  ####################################")

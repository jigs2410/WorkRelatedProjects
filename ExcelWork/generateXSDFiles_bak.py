    ...
    for row in Sheet1.rows:
        ...
        #Similarly if par value has changed that previous par value along with its child is pushed to pars collection
            ...
            if (not (row[8].value is None) and not (row[9].value is None)) or len(Flag)>0:
                #print ("Came 1",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                if ( not (CurrFlagsPosition) is None): #\
                    #or (CurrDevices!=DeviceList and rowCount!=2):
                    #print ("Came in",rowCount,Flag,Flags)
                    #print ("Came 2",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                    if (len(Flag)>0):
                        for Device in AllDevices:
                            if Device in CurrDevices:
                                ...
                                exec("%s_Flag.clear()"%Device)
                                #exec("print (rowCount,Device,\" Flags->\",%s_Flags)"%Device)
                        #print ("Came 3",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)           
                        Flags[CurrFlagsPosition]=Flag.copy()
                        Flag.clear()
                        #print ("Came 4",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                if not (row[8].value is None):
                    CurrFlagsPosition=row[8].value
                    Flag['BitID']= row[8].value
                    Flag['BitName']=row[9].value
                #print ("Came 5",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
            ...
            if (not (row[6].value is None) and not (row[7].value is None)):
                #print ("Came 6",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                if (CurrParam!=row[6].value and not (CurrParam is None)) \
                    or (CurrDevices!=DeviceList and rowCount!=2):
                    #print ("Came 7",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                    def forallFlagDevices(Device_Flags,Device_Flag, Device_Param, Device_Params):
                        Device_Param = Param.copy()
                        if (len(Device_Flags)>0):
#                            if (not (row[8].value is None) and len(Device_Flag)>0) :
...
#                                Device_Flag.clear()
                        ...
                        Device_Param.clear()
                        #exec("print (rowCount,Device,\" Params->\",%s_Params)"%Device)
                    ...
                    if (len(Flags)>0):
                        #print ("Came 8",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
#                        if (not (row[8].value is None) and len(Flag)>0):
#                            print ("Came 9",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
#                            Flags[CurrFlagsPosition]=Flag.copy()
#                            Flag.clear()
#                            print ("Came 10",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                        Param['Flags']=Flags.copy()
                        Flags.clear()
                        #CurrFlagsPosition=None
                        #print ("Came 11",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
                ...
                if not (row[12].value is None):
                    ...
                    Param['RD_ID']='RD_0000'
            #print ("Came 12",rowCount," CurrFlagsPosition->",CurrFlagsPosition," Flag->",Flag," Flags->",Flags)
...
#                            exec("print (rowCount,Device,\" Pars->\",%s_Pars)"%Device)
    ...
    for Device in AllDevices:
        exec("print (rowCount,Device,\" Zone->\",%s_Zone)"%Device)
        print()
        exec("print (rowCount,Device,\" Zones->\",%s_Zones)"%Device)
        print()
        exec("print (rowCount,Device,\" Par->\",%s_Par)"%Device)
        print()
        exec("print (rowCount,Device,\" Pars->\",%s_Pars)"%Device)
        print()
        exec("print (rowCount,Device,\" SubPars->\",%s_SubPars)"%Device)        
        print()
        exec("print (rowCount,Device,\" SubPar->\",%s_SubPar)"%Device)        
        print()
        exec("print (rowCount,Device,\" Params->\",%s_Params)"%Device)        
        print()
        exec("print (rowCount,Device,\" Param->\",%s_Param)"%Device)        
        
    if(rowCount == row_count):
        CurrDevices=DeviceList
        #print("CurrDevices",CurrDevices)
...
#            print("Entering all lastline \n\n\n")
#            print("Device",Device)
#            print("Device_Flag",Device_Flag)
#            print("Device_Flags",Device_Flags)
#            print("Device_Param",Device_Param)
#            print("Device_Params",Device_Params)
#            print("Device_SubPar",Device_SubPar)
#            print("Device_SubPars",Device_SubPars)
#            print("Device_Par",Device_Par)
#            print("Device_Pars",Device_Pars)
#            print("Device_Zone",Device_Zone)
#            print("Device_Zones",Device_Zones)
#            print()
...
#        print()
        for Device in AllDevices:
            print()
            print(Device)
            #exec("print(\"%s\",%s_Zones)" % (Device,Device))

  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "MSSQL"."LOWES_PRICING" AS 
...
----
lv_colorpricetype OPTION_COLOR_LOWES.PRICETYPE%type;
lv_gridpricetype OPTION_GRID_LOWES.pricetype%TYPE;
lv_gridpatternpricetype OPTION_GRID_LOWES.pricetype%TYPE;
lv_gridcolorpricetype OPTION_GRID_LOWES.pricetype%TYPE;
...
END LOWES_PRICING;
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "MSSQL"."LOWES_PRICING" AS
...
BEGIN
   ...
   insert into new_pricinglog (orderid,itemid,unitid,linenumber,linetypeid,lognexcept,appliedon,optiontableid,price,stdsize, isshape,hasgrid, 
                              screendeduct,ispart,sizetype,orderwidth,orderheight,mulledunits,effectiveunit,sections,totalunit) 
                              values
                              ...
                              lv_screendeduct,lv_part,lv_sizetype,lv_widthes,lv_heightes,lv_mulledunits,lv_effectiveunit,lv_sections,lv_totalunits);
...
if (lv_price >0 ) then
  begin
   ...
   nvl(multvalue,'1')
    ...
    and (regexp_like (style,''''||G_STYLE||'''','i') or style = '-')
    and (regexp_like (series,''''||G_SERIES||'''','i') or series = '-')
...
lv_price number :=0;
lv_pricetype option_warranty_lowes.pricetype%TYPE;
--lv_pricemul option_warranty_lowes.pricemul%type;
lv_warrantyid option_warranty_lowes.ID%type;
lv_warranty_desc option_warranty_lowes.warranty_desc%type;
...
BEGIN
  ...
  begin
     ...
     select price,pricetype,id  into lv_price,lv_pricetype,lv_warrantyid from
      (select price,pricetype,pricemul,id  from  option_warranty_lowes
      where 
...
lv_tint  option_glass_lowes.TINT%TYPE;
lv_tintpricetype option_glass_lowes.TINT_PRICE_TYPE%TYPE;
lv_energypricetype option_glass_lowes.energy_PRICE_TYPE%TYPE;
lv_tintsafety option_glass_lowes.SAFETY%TYPE;
lv_safetypricetype option_glass_lowes.safety_PRICE_TYPE%TYPE;


lv_calc_safety number :=1;

cursor c_tint is 
select id, nvl(tint_price,0), tint_price_type,safety,nvl(safety_price,0), safety_price_type from option_glass_lowes
...
lv_glassprice :=0;
  ...
  begin 
    ...
    EXIT WHEN c_tint%NOTFOUND;
      if lv_safetyprice is not null or lv_tintin like '%Tempered' or lv_tintin like '%TEMPERED' then
  ...
  begin
    ...
    select id,nvl(energy_price,0), energy_price_type into  lv_energyID,lv_energyprice,lv_energypricetype from (
    select id,energy_price, energy_price_type   from option_glass_lowes
    where regexp_like(energy,lv_energy,'i') and regexp_like(pane_type,''''||lv_panetype||'''','i')
    and (regexp_like (series,''''||lv_SERIES||'''','i') or series = 'ALL')
    and (regexp_like (style,''''||lv_Style||'''','i') or style = 'ALL') and nfrc =lv_NFRC  order by series,style) x where rownum < 2;
    EXCEPTION WHEN NO_DATA_FOUND THEN
    begin 
      lv_energyprice := 0;
      lv_energyID:=0;
...
if (lv_calc_safety = 1 and lv_safety not in ('Single Strength','SINGLE STRENGTH')) then 
   begin 
      ...
      select id,safety_price, safety_price_type from option_glass_lowes
...
lv_pricemul option_grid_lowes.pricemul%TYPE;
--lv_lites number := 0;
lv_gridpatternid number :=0;
lv_gridpatternprice number :=0;
...
if (lv_isSHAPE = 1) then
      ...
      lv_price :=0;
        ...
        select * INTO lv_GRIDID,LV_PRICE , LV_gridPRICETYPE ,LV_PRICEMUL from (select id, price , pricetype, pricemul from  option_grid_lowes
...
--                end if;
                 ...
                 if (lv_isSHAPE = 0) then --tso bso etc applies only 
                    begin 
                      select id,price,pricetype into lv_gridpatternid, lv_gridpatternprice, lv_gridpatternpricetype from option_grid_lowes 
                      where regexp_like ( construction,''''||LV_CONSTRUCTION||'''','i')
                      and regexp_like (gridstyle,''''||LV_GRIDSTYLE||'''','i') 
                      and regexp_like (pattern,''''||LV_GRIDPATTERN||'''','i') ;
                ...
                if (LV_GRIDCOLOROUT not in ('White','WHITE') ) then
                
                  begin 
                        select id,price,pricetype into lv_gridcolorid, lv_gridcolorprice, lv_gridcolorpricetype from option_grid_lowes 
                        where regexp_like ( construction,''''||LV_CONSTRUCTION||'''','i')
                        and gridstyle= 'ALL' 
                        and pattern ='ALL' 
                        and shapes = lv_isshape
                        and regexp_like  (gridcolorout,''''||LV_GRIDCOLOROUT||'''','i');
...
lv_clear                    new_pricingsize_lowes.clear%type;
lv_clear_pricetype          new_pricingsize_lowes.clear_Pricetype%type;
lv_withgrid                 new_pricingsize_lowes.withgrid%type;
lv_withgrid_pricetype       new_pricingsize_lowes.withgrid_pricetype%type;
lv_screen_deduct            new_pricingsize_lowes.screen_deduct%type;  
lv_screen_deduct_pricetype  new_pricingsize_lowes.screen_deduct_pricetype%type;
lv_clear_minui              new_pricingsize_lowes.clear_minui%type;
lv_withgrid_minui           new_pricingsize_lowes.withgrid_minui%type;
 ...
 ---FOR NEW CONSTRUCTION UNited inches  = CEIL(WIDTHES) + CEIL(HEIGHTES) and for replacement  floor(...
    ...
    if (lv_isSHAPE !=1) then       
          ...
          if (LV_CONSTRUCTION = 'N') THEN
            begin --1
                  ...
                  from new_pricingsize_lowes where width = LV_WIDTHES and height=in_heightes 
                  ...
                  begin --2
                      ...
                      begin --4  -- try united inchees now.
                          ...
                          from new_pricingsize_lowes where ui= 1
                          and  regexp_like ( MODEL,''''||lv_model||'''','i') and series ='0' and style ='0';
                 ...
                 lv_screen_deduct := lv_effectiveunit * lv_screen_deduct;
                  ...
                  if (lv_part = 1) then
                    ...
                    if (lv_sections >=3) then --only baybow
                      begin--1
                        select sizeid,bb_price into lv_sizeid,lv_baseprice  from new_pricingsize_lowes 
                          ...
                          EXCEPTION WHEN NO_DATA_FOUND THEN           
                            ...
                            begin --1
                               select sizeid,bracketprice into lv_sizeid,lv_baseprice  from new_pricingsize_lowes 
                                where regexp_like ( MODEL,''''||lv_model||'''','i') and style ='0' and series ='0'
                                and lowerui < lv_unitedinches and upperui >= lv_unitedinches;
                                lv_sizetype :='REPL_BRACKET';
                                 EXCEPTION WHEN NO_DATA_FOUND THEN
                                  begin --2
                                  select sizeid,maxuiprice + (lv_unitedinches - maxui)*maxperui into lv_sizeid,lv_baseprice from new_pricingsize_lowes 
                                  where regexp_like ( MODEL,''''||lv_model||'''','i') and style ='0' and series ='0'
  ...
  --end if;
   ...
   begin --1
                  ...
                  from new_pricingsize_lowes where width = LV_WIDTHES and height=in_heightes  and regexp_like(tripleeyebrow, lv_tripleeyebrow)
                  and  regexp_like ( MODEL,''''||lv_model||'''','i') and series ='0' and style ='0';
                  lv_stdsize := 1;
...
lv_price option_color_lowes.price%type;

lv_pricemul option_color_lowes.PRICEMUL%type;
...
----NOTE:- ANYTHING ABOVE MULLEDUNITS 1 has different set of rule
  ...
  select colorID,price ,pricetype,pricemul into lv_COLORID,lv_price,lv_colorpricetype,lv_pricemul from (
   select colorID,price ,pricetype,pricemul  from  option_color_lowes
...
--            begin
            select * into lv_WOOD_MULLID,lv_price from (select ID, case 
                        ...
                        end  from option_wood_mull_lowes
...
--custom size u add 20$ additional per effective unit
  if (lv_STDSIZE != 1) and (lv_price >0 ) then
    BEGIN 
       
       SELECT UNIT_1  INTO LV_WOODCUSTOMADDON  FROM (SELECT UNIT_1 from option_wood_mull_lowes
        where regexp_like (woodsurr_mulltype , ''''||IN_WOOD_MULL||'''','i')
        and (REGEXP_LIKE (style ,''''||lv_STYLE||'''','i') or style = 'ALL')
...
lv_price number :=0;
lv_pricetype option_seat_lowes.price_type%TYPE;
--lv_pricemul option_seat_lowes.pricemul%type;
lv_seatid option_seat_lowes.ID%TYPE;


BEGIN

  begin
    --select * into lv_price,lv_pricetype,lv_pricemul,lv_seatid from ( select price,price_type,pricemul,id  from option_seat_lowes
    select * into lv_price,lv_pricetype,lv_seatid from ( select price,price_type,id  from option_seat_lowes
...
lv_price number :=0;
lv_pricetype option_screenmesh_lowes.price_type%TYPE;
lv_pricemul option_screenmesh_lowes.pricemul%type;
lv_section option_screenmesh_lowes.sections%type ;
lv_screenmeshid option_screenmesh_lowes.ID%TYPE;
--lv_screentype option_screenmesh_lowes.screentype%TYPE;
...
BEGIN
  ...
  begin
  select * into lv_price,lv_pricetype,lv_pricemul,lv_screenmeshid from ( select price,price_type,pricemul,id  from option_screenmesh_lowes 
  where regexp_like (constructioncode,''''||lv_construction||'''','i')
  and
...
lv_price number :=0;
lv_pricetype option_cottageoriel_lowes.price_type%TYPE;
lv_pricemul option_cottageoriel_lowes.price_mul%type;
lv_cottageorielid option_cottageoriel_lowes.ID%type;

BEGIN


begin
  select price,price_type,price_mul,id  into lv_price,lv_pricetype,lv_pricemul,lv_cottageorielid from  option_cottageoriel_lowes
  where 
...
lv_price number :=0;
lv_pricetype option_foamwrap_lowes.price_type%TYPE;
lv_pricemul option_foamwrap_lowes.pricemul%type;
lv_foamwrapid option_foamwrap_lowes.ID%type;
...
BEGIN
  ...
  (select price,price_type,pricemul,id  from  option_foamwrap_lowes
  where 
  type= lv_foamwrap
  and (regexp_like (series,''''||LV_SERIES||'''','i') or series = 'ALL')
...
--lv_sections  option_hardware_lowes.sections%type;
lv_pricetype option_hardware_lowes.price_type%TYPE;
--lv_pricemul option_hardware_lowes.pricemul%type;

lv_hardwareid option_hardware_lowes.ID%type;
lv_accessoryid option_hardware_lowes.ID%type;
lv_egressid option_hardware_lowes.ID%type;
...
--  else lv_sections := 2; end if;
  ...
  begin --1
    select price,price_type,id  into lv_hardwaretype_colorprice,lv_pricetype,lv_hardwareid from option_hardware_lowes 
...
--NOW get accessory
  ...
  if (lv_hardwareACCESSORY!='0') then
    begin
      select price,price_type,id  into lv_hardwareaccessoryprice,lv_pricetype,lv_accessoryid from option_hardware_lowes 
  ...
  if (lv_EGRESS!='0') then
      begin
        select price,price_type,id  into lv_hardwareegressprice,lv_pricetype,lv_egressid from option_hardware_lowes 
...
lv_price := lv_hardwaretype_colorprice + lv_hardwareaccessoryprice + lv_hardwareegressprice;
  ...
  if (lv_hardwareprice!=0) then proc_WriteToPricingLog ('HARDWARE',0,lv_price); end if;
...
lv_price number :=0;
lv_pricetype option_drywall_lowes.pricetype%TYPE;
lv_pricemul option_drywall_lowes.pricemul%type;
lv_drywallid option_drywall_lowes.ID%type;


BEGIN

begin
  select * into lv_price,lv_pricetype,lv_pricemul,lv_drywallid from ( select price,pricetype,pricemul,id  from option_drywall_lowes
...
lv_price number :=0;
lv_pricetype option_sashaccessory_lowes.pricetype%TYPE;
--lv_pricemul option_sashaccessory_lowes.pricemul%type;
lv_sashaccessoryid option_sashaccessory_lowes.ID%type;


BEGIN
  begin
      select * into lv_price,lv_pricetype,lv_sashaccessoryid from ( select price,pricetype,id  from option_sashaccessory_lowes
...
lv_price number := 0;
lv_ID number := 0;
lv_otherspricetype option_others_lowes.pricetype%TYPE;

BEGIN
  begin 
    select price,id,pricetype into lv_price,lv_id,lv_otherspricetype from 
    (select price ,id,pricetype from option_others_lowes 
    where optionname = IN_optionname
    
--and ( case when ( IN_optionname = '') then (1)
--            when ( :p1_linenumber is null ) then (1)
--            else 0 end)=1
    and regexp_like (value,''''||IN_VALUE||'''','i')
    and (regexp_like (style,''''||lv_STYLE||'''','i') or style = 'ALL' )
    and (regexp_like (series,''''||lv_SERIES||'''','i') or series = 'ALL' )
    
    and ( ( IN_OPTIONNAME = 'LOCKS' and (widthes ! = 1 or heightes != 1 ) ) or (widthes = 1 and heightes = 1))
    and ( ( IN_OPTIONNAME = 'FINS' and mulledunits = lv_mulledunits ) or (mulledunits=0))
    ) x where rownum < 2;
...
lv_price number :=0;
lv_pricetype option_screen_lowes.price_type%TYPE;
--lv_pricemul option_screen_lowes.pricemul%type;
lv_section option_screen_lowes.sections%type ;
lv_screenid option_screen_lowes.ID%TYPE;
...
begin
  select * into lv_price,lv_pricetype,lv_screenid from ( select price,price_type,id  from option_screen_lowes 
  where regexp_like (constructioncode,''''||lv_construction||'''','i')
...
--    end if;
    ...
    if (lv_pricetype = 'TOTALPERC') then
      ...
      proc_WriteToPricingLog ('FIXED SCREEN',lv_screenid,lv_price);
      
      --sum everything so far and do percent on that.
      
      lv_price := - (
      (lv_baseprice + lv_gridprice+lv_colorprice+lv_glassprice+lv_wood_mullprice+lv_seatprice +lv_cottageorielprice + lv_foamwrapprice + lv_spacerprice+lv_lockprice+lv_drywallprice+lv_breathertubeprice+lv_sashaccessoryprice +lv_smrprice +lv_screenmeshprice + lv_warrantyprice+lv_finprice)
...
--         
   ...
   if (lv_totalperc!=0) then
        lv_finalprice :=lv_baseprice + lv_gridprice+lv_colorprice+lv_glassprice+lv_wood_mullprice+lv_seatprice +lv_cottageorielprice+ lv_foamwrapprice+ lv_spacerprice+lv_lockprice+lv_drywallprice+lv_breathertubeprice+lv_sashaccessoryprice+lv_smrprice+lv_screenmeshprice+lv_warrantyprice+lv_finprice;
...
--  proc_WriteToPricingLog ('BREATHERTUBEPRICE',0,lv_BREATHERTUBEprice);
    ...
    lv_finalprice :=lv_baseprice + lv_gridprice+lv_colorprice+lv_glassprice+lv_wood_mullprice+lv_seatprice+lv_screenprice +lv_cottageorielprice+ lv_foamwrapprice+ lv_spacerprice+lv_lockprice+lv_drywallprice+lv_breathertubeprice+lv_sashaccessoryprice+lv_smrprice+lv_screenmeshprice + lv_warrantyprice+lv_finprice;
...
begin
  ...
  --L1_item
    select construction,rendercode,nvl(woodsurround,0),linenumber,nvl(seat,'0') , nvl(foamwrap,'0'),nvl(gridplacement,'0'),nvl(fins,0)
    into lv_construction,lv_rendercode,lv_woodsurround,lv_linenumber,lv_seat ,lv_foamwrap,lv_gridplacement,lv_fins
  ...
  --GET sash and frameid.
    ...
    select case when exists(SELECT STYLE
                         from option_grid_lowes where regexp_like (style,''''||lv_style||'''','i') and shapes =1)
           then 1
           else 0
...
--  
 ------Write final atrium price
  lv_atriumprice :=lv_baseprice + lv_gridprice+lv_colorprice+lv_glassprice+lv_wood_mullprice + lv_screenprice+lv_seatprice +lv_cottageorielprice+ lv_foamwrapprice+ lv_spacerprice+lv_lockprice+lv_drywallprice+lv_breathertubeprice
  +lv_sashaccessoryprice+lv_smrprice+lv_screenmeshprice+lv_warrantyprice+lv_finprice;  
-- 
--  

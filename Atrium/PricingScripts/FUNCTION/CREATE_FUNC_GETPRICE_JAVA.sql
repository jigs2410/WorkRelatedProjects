  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "MSSQL"."FUNC_GETPRICE_JAVA" 
( p_UNITID IN NUMBER,
p_DEBUG IN NUMBER
  ...
  l1_item.linenumber,
         ...
         --' ', --NVL(FUNC_AGGREGATE_GRIDPATTERNS(p_UNITID),' '),
          ...
          l5_glass.caelusglasslocation,
            ...
            --,L2_UNIT.DRYER_VENT
           ,0 --,NVL(func_getlites(NVL(FUNC_AGGREGATE_GRIDPATTERNS(p_UNITID),' ')),0)
            ,l2_unit.smr
            ,0--NVL(func_isshape(l2_unit.style),0)
            ,0--NVL(func_pricecustom('',upper(l2_unit.style),upper(l5_glass.gridpattern),1,0,'GRIDPATTERN'),0)
            ,0--NVL(func_pricecustom('',upper(l2_unit.style),upper(l5_glass.gridpattern),0,1,'GRIDPATTERN'),0)
           ,0--NVL(func_pricecustomsize(''''||l2_unit.model||'''',l2_unit.widthes,l2_unit.heightes,1,0, style.orcl_effect_unit),0)
            ,0--NVL(func_pricecustom('',upper(l2_unit.style),upper(l3_Screen.screentype),1,0,'SCREEN'),0)
            ,0--,NVL(func_get_mult(ordermaster.accountid,ordermaster.customerid,l2_unit.series,l2_unit.style,l2_unit.model,100),0)
            ,l1_item.itemid
            ,style.category
            ,style.part
            ,0--NVL(func_pricecustom('','',upper(l3_frame.framecolorin),0,0,'INTERIORLAMINATE'),0)
            ,0--NVL(func_pricecustom('','',upper(l3_frame.framecolorout),0,0,'EXTERIORPAINT'),0)
            ,0--NVL(func_pricecustom(l2_unit.model,'','',0,0,'DOORPANEL'),0)
...
--DBMS_OUTPUT.PUT_LINE(lv_pricinggroupid || ' '||lv_precondition);
  EXIT WHEN cur_Pricing%NOTFOUND;

--EXECUTE IMMEDIATE lv_precondition;
lv_return:=1; -- we still want to apply the pricing if no precondition exists
IF (p_DEBUG=1) THEN 
...
--DBMS_OUTPUT.PUT_LINE('3');
  ...
  ----------
  -----Custom parameter data assignment---
  ---------
  --lv_lites
--  if (lv_precondition like '%lites%' or lv_prevalue like '%lites%') then
--    select NVL(func_getlites(NVL(FUNC_AGGREGATE_GRIDPATTERNS(p_UNITID),' ')),0) into lv_lites from dual;
--  end if;
--  
--  --lv_gridpattern
--  if (lv_precondition like '%gridpattern%' or lv_prevalue like '%gridpattern%') then
--    select NVL(FUNC_AGGREGATE_GRIDPATTERNS(p_UNITID),' ') into lv_gridpattern from dual;
--  end if;
--  
--  if (lv_precondition like '%shapes%' or lv_prevalue like '%shapes%') then
--    select NVL(func_isshape(lv_style),0) into lv_shapes from dual;
--  end if;
--  
--  if (lv_precondition like '%shapeprice%' or lv_prevalue like '%shapeprice%') then
--    select NVL(func_pricecustom('',upper(lv_style),upper(lv_gridpattern),1,0,'GRIDPATTERN'),0) into lv_shapeprice from dual;
--  end if;
--  
--  if (lv_precondition like '%shapesdlprice%' or lv_prevalue like '%shapesdlprice%') then
--    select NVL(func_pricecustom('',upper(lv_style),upper(lv_gridpattern),0,1,'GRIDPATTERN'),0) into lv_shapesdlprice from dual;
--  end if;
--  
  if (lv_precondition like '%stdsize%' or lv_prevalue like '%stdsize%') then
    select NVL(func_pricecustomsize(''''||lv_model||'''',lv_widthes,lv_heightes,1,0, lv_effectiveunit),0) into lv_standardsize from dual;
  end if;
  
--    if (lv_precondition like '%screenprice%' or lv_prevalue like '%screenprice%') then
--    select NVL(func_pricecustom('',upper(lv_style),upper(lv_screentype),1,0,'SCREEN'),0) into lv_screenprice from dual;
--  end if;   
--    
--    if (lv_precondition like '%laminate%' or lv_prevalue like '%laminate%') then
--    select NVL(func_pricecustom('','',upper(lv_framecolorin),0,0,'INTERIORLAMINATE'),0) into lv_laminate from dual;
--  end if;  
--  
--    if (lv_precondition like '%painted%' or lv_prevalue like '%painted%') then
--    select NVL(func_pricecustom('','',upper(lv_framecolorout),0,0,'EXTERIORPAINT'),0) into lv_painted from dual;
--  end if;    
--            
--    if (lv_precondition like '%doorpanel%' or lv_prevalue like '%doorpanel%') then
--    select NVL(func_pricecustom(lv_model,'','',0,0,'DOORPANEL'),0) into lv_doorpanel from dual;
--  end if;
...
--                          :lv_return := 0;
--                        END IF;
--                          SELECT ' || lv_prevalue || ' INTO :nbr_prevalue FROM DUAL;
--                       --:nbr_prevalue := :111;
...
--                     ;
--ELSE
--if lv_prevalue IS NOT NULL
--THEN
--EXECUTE IMMEDIATE 'DECLARE
--              widthes l2_unit.widthes%TYPE := :1;
--              heightes l2_unit.heightes%TYPE := :2;
--              widededuct  NUMBER(5,3):=:3;
--              highdeduct  NUMBER(5,3):=:4;
--              effectiveunit NUMBER := :5;
--
--              ui NUMBER := widthes+heightes+widededuct+highdeduct;
--
--              -- To conform to EMI rounding, need to always round up to the next full inch with fractions.
...
--if condition is true or there was no condition but a database hit, we need to apply pricing
  ...
  if (lv_precondition like '%ui_es%' ) then
    lv_precondition := replace(lv_precondition, 'ui_es', ' '||CEIL(lv_widthes) + CEIL(lv_heightes)|| ' ');
  end if;
  
    if (lv_precondition like '%stdsize%' or lv_prevalue like '%stdsize%') then
     lv_precondition := replace (lv_precondition, 'stdsize' , ' '|| lv_standardsize||' ' );
  end if;
  
   DBMS_OUTPUT.PUT_LINE (lv_precondition);
   
  --select to_number(nvl( evalCond( lv_precondition ),0)) into lv_return  from dual;     
  select nvl( evalCond( lv_precondition ),0) into lv_return  from dual;     
